...
FollowTargetTilStopped::FollowTargetTilStopped(float Speed, bool Pickup) {
...
void FollowTargetTilStopped::Initialize() {
...
void FollowTargetTilStopped::Execute() {
  ...
  if (target.Detected == true) //- for now, pursue all targets same way
  {
    ...
    if (TargetAngle >=0.0)
      ...
      Robot::m_MainDrive.SetArcadeDrive(-targetSpeed, std::min(0.3, 0.02*TargetAngle), false);
    
    else 
      // drive towards target
      Robot::m_MainDrive.SetArcadeDrive(-targetSpeed, std::max(0.02*TargetAngle, -0.3), false);
...
bool FollowTargetTilStopped::IsFinished() { 
...
void FollowTargetTilStopped::End() {
    Robot::m_MainDrive.SetArcadeDrive(0.0, 0.0 , false);
}
// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void FollowTargetTilStopped::Interrupted() {
