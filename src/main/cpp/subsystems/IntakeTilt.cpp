...
IntakeTilt::IntakeTilt() {
  ...
  m_Motor = new WPI_TalonSRX(INTAKE_TILT_MOTOR_CANID);
  ...
  m_Motor->ConfigPeakOutputForward (INTAKE_TILT_DRIVE_FULL_VLTG_FWD, 0);
  m_Motor->ConfigPeakOutputReverse (-INTAKE_TILT_DRIVE_FULL_VLTG_REV, 0);

  // Set elevator motor maximum current level
  m_Motor->ConfigContinuousCurrentLimit(INTAKE_TILT_CURRENT_LIMIT);
  m_Motor->ConfigPeakCurrentLimit(INTAKE_TILT_PEAKCURRENT_LIMIT);
  m_Motor->ConfigPeakCurrentDuration(INTAKE_TILT_PEAKCURRENT_TIME);
  ...
  m_Motor->ConfigForwardSoftLimitThreshold(INTAKE_TILT_SOFT_LIMIT_MAX);
  ...
  m_Motor->ConfigReverseSoftLimitThreshold(INTAKE_TILT_SOFT_LIMIT_MIN); 
  ...
  m_Motor->Config_kP(0, INTAKE_TILT_PGAIN, 0);
  m_Motor->Config_kI(0, INTAKE_TILT_IGAIN, 0);
  m_Motor->Config_kD(0, INTAKE_TILT_DGAIN, 0);

  // set integration maximum accumulator value
  m_Motor->ConfigMaxIntegralAccumulator(0, INTAKE_TILT_MAX_INTEGRAL_ACCUMULATOR, 0);
 
  // Set maximum allowed posotion controller closed loop error 
  m_Motor->ConfigAllowableClosedloopError(0, INTAKE_TILT_ALLOWABLE_CLOSEDLOOP_ERROR, 0);
...
void IntakeTilt::SetIntakeTiltTargetPosition(int pos)
{
  ...
  switch (pos) {
    case 0:
      m_Motor->Set(ControlMode::Position, INTAKE_TILT_UP_POSITION + (int)PosUpAdjust.GetDouble(0.0));
      break;

    case 1:
      m_Motor->Set(ControlMode::Position, INTAKE_TILT_MID_POSITION + (int)PosMidAdjust.GetDouble(0.0));
      break;

    case 2:
      if (m_AdjustForBottom == true)
        m_Motor->Set(ControlMode::Position, INTAKE_TILT_BALL_POSITION + (int)PosBallAdjust.GetDouble(0.0));
      else
        m_Motor->Set(ControlMode::Position, INTAKE_TILT_DOWN_POSITION + (int)PosDownAdjust.GetDouble(0.0));
      break;
...
bool IntakeTilt::IsIntakeTiltAtTarget(void){
  ...
  if (abs(m_Motor->GetClosedLoopError()) <= INTAKE_TILT_POSITION_TOLERANCE )
    return true;
  else
    return false;
}
...
{  
  ...
  int target = m_Motor->GetClosedLoopTarget(0);
   ...
   // depending on position given to us, tell motor to turn to desired position (in # of pulse counts)
  if (target==INTAKE_TILT_UP_POSITION + (int)PosUpAdjust.GetDouble(0.0))
    pos = 0;
  
  else if (target==INTAKE_TILT_MID_POSITION + (int)PosMidAdjust.GetDouble(0.0))
    pos = 1;

  else if (target==INTAKE_TILT_DOWN_POSITION + (int)PosDownAdjust.GetDouble(0.0))
    pos = 2;

  else if (target==INTAKE_TILT_BALL_POSITION + (int)PosBallAdjust.GetDouble(0.0))
    pos = 2;
...
void IntakeTilt::SetIntakeTiltTargetAnalog(int pos){
  ...
  if (pos<INTAKE_TILT_SOFT_LIMIT_MIN )
      pos=INTAKE_TILT_SOFT_LIMIT_MIN;
  if (pos>INTAKE_TILT_SOFT_LIMIT_MAX)
      pos=INTAKE_TILT_SOFT_LIMIT_MAX;
...
void IntakeTilt::InitializeShuffleBoard(void)
{
    ...
    l1->WithPosition(3,0);
    l1->WithSize(1,2);
    l1->Add("Up", INTAKE_TILT_UP_POSITION);
    l1->Add("Mid", INTAKE_TILT_MID_POSITION);
    l1->Add("Down", INTAKE_TILT_DOWN_POSITION);
    l1->Add("Ball", INTAKE_TILT_BALL_POSITION);
